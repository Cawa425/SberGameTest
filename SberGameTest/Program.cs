    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            while (true)
            {
                ...
                var rightPlacesCharsIds = string.IsNullOrEmpty(rightPlacesCharsIdsInput)? new List<int>(): rightPlacesCharsIdsInput.Split(' ').Select(int.Parse).ToList();
                Console.WriteLine("Какие буквы синие? (номер букв в слове через пробел )");
                var wrongPlacesCharsIdsInput = Console.ReadLine();
                var wrongPlacesCharsIds = string.IsNullOrEmpty(wrongPlacesCharsIdsInput)? new List<int>(): wrongPlacesCharsIdsInput.Split(' ').Select(int.Parse).ToList();
                var totalyWrongChars = inputWord.ToList().Where(x=> !rightPlacesCharsIds.Contains(inputWord.IndexOf(x)+1) && 
                                                                    !wrongPlacesCharsIds.Contains(inputWord.IndexOf(x)+1)).ToList();
                
                Console.WriteLine("Какие буквы повторяются? (номер буквы и количество в слове через пробел, например 1 2. Несколько букв размечать запятой)");
